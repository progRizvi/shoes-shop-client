...
const useFirebase = () => {
        ...
        createUserWithEmailAndPassword(auth, email, password, name)
            .then((userCredential) => {
                updateProfile(auth.currentUser, {
                    displayName: name,
                }).then(() => { handleUser(email, name) }).catch((error) => {});
    ...
    const LoginWithEmail = (email, password, redirectUrl) => {
        ...
        signInWithEmailAndPassword(auth, email, password)
            .then((userCredential) => {
                ...
                setTimeout(() => { history.replace(redirectUrl) }, 1500)
            })
            .catch((error) => {
                setError(error.code)
    ...
    useEffect(() => {
        axios
            .get(`https://squadrone.herokuapp.com/user/?email=${user.email}`)
            .then((result) => {
    ...
    const handleUser = (userEmail, userName) => {
        const url = "https://squadrone.herokuapp.com/users"
        const role = "user";
        const userInfo = { userEmail, userName, role }
        axios.post(url, userInfo).then(result => {})
    }
    return { registerWithEmail, logOut, user, isLoading, successMessage, error, LoginWithEmail, isAdmin, userInfo }
}
